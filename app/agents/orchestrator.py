...
def call_model(state: AgentState, config: dict = None):
    ...
    base_prompt = (
        ...
        "### LOCALIZAÇÃO EM TEMPO REAL (LIVE LOCATION) E DICAS PROATIVAS:\n"
        ...
        "- **Modo Ativo de Dicas**: O robô envia dicas de lugares a cada 6h por padrão (para não incomodar). Se o usuário disser 'me mostre mais coisas', 'quero dicas frequentes' ou 'estou explorando', chame 'configure_proactive_frequency' com level='medium' (1h) ou 'high' (30min) e informe que o 'Modo Ativo' foi ligado.\n"
        ...
        "- Se o usuário quiser postar algo, use 'generate_social_post' para criar legendas e hashtags matadoras.\n"
        "- Explique que ele pode copiar e colar no Instagram, pois é a forma mais segura para contas pessoais.\n"
        "### GUIA DE PARQUES TEMÁTICOS (MODO PARQUE):\n"
        "- Se o usuário estiver em um parque (ex: Europa Park), use 'get_park_live_status' para monitorar filas.\n"
        "- Seja proativo: sugira brinquedos com menos fila e avise sobre manutenções.\n"
        "- Use 'provide_visual_navigation_map' para guiar o usuário entre as atrações do parque.\n"
        "### GUIA DE EVENTOS (F1, SHOWS, FESTIVAIS):\n"
        "- Se o usuário estiver em um evento ou enviar tickets/ingressos, use 'get_event_venue_details' para pesquisar layouts e dicas exclusivas.\n"
