...
import os  # [CORREÇÃO 5] Import adicionado ao topo
import httpx
import asyncio
...
from app.services.user_service import UserService  # Import global para evitar NameError
...
async def chat_endpoint(
    ...
    # [CORREÇÃO 2 e 3] Instanciação e captura do ID Original
    ...
    try:
        logger.info(f"📡 [INCOMING] De: {user_id_raw} -> {request.user_id}")
...
async def process_chat_message(request: ChatRequest, agent: TravelAgent, idempotency_key: str):
    ...
    async with get_lock(request.user_id):
        ...
        try:
            ...
            # [CORREÇÃO 4] ACL Unificada (Dicionário)
...
async def media_webhook(request: MediaRequest, background_tasks: BackgroundTasks):
    user_service = UserService() # [CORREÇÃO 2]
    idempotency = get_idempotency()
...
async def process_media_webhook(request: MediaRequest, idempotency_key: str):
    ...
    async with get_lock(request.user_id):
        ...
        try:
            ...
            # Simulação de payload para o ingestor
            data_payload = {
            ...
            # [CORREÇÃO 1] Indentação da Relevância
            is_travel = result.get("is_travel_content", True)
            if not is_travel or result.get("status") == "irrelevant":
                warning_msg = f"🤖 O documento *{request.filename}* não parece ser de viagem. Incluir mesmo assim? (sim/não)"
                user_service.set_pending_irrelevancy(request.user_id, result)
                N8nService().enviar_resposta_usuario(request.user_id, warning_msg, bypass_firewall=True)
                return  # Para aqui se for irrelevante

            # Se for relevante, segue para o sucesso
...
# Endpoints de Dashboard e Mapa mantidos de forma concisa para estabilidade
@router.post("/dashboard/action")
async def dashboard_action(request: Dict[str, Any]):
    return {"success": True}
